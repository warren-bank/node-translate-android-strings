...
const get_output_text_translation = function(translations) {
...
const process_cli = async function(argv_vals){
  ...
  for (let i=0; i < all_lines.length; i++) {
    ...
    line.replace(regex.string, function(xml, value){
      ...
      if (regex.exclude.has_entity_value.test(value))
        return ''

      if (regex.exclude.has_string_value.test(value))
        return ''

      translatable.string.push({index: i, value})
      return ''
    })
  }
  ...
  for (let i=0; i < argv_vals["--output-language"].length; i++) {
    ...
    const text_output = get_output_text_translation(translated_strings_array)
