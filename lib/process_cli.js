...
const split_translatable_value = function(value, separator = regex.split.all) {
  return value.split(separator).filter(chunk => chunk && !regex.filter.punctutation.test(chunk)).map(chunk => chunk.trim())
}
...
const get_output_text_translation = function(argv_vals, translations) {
  if (translations.length !== (translatable.entity.length + translatable.string.length)) {
    if (argv_vals["--debug"]) {
      console.log('ERROR:', 'Number of translated strings is incorrect!', `Expected #${translatable.entity.length + translatable.string.length} but received #${translations.length} from server.`, {translations})
      throw new Error('')
    }
    else {
  ...
  for (let i=0; i < all_lines.length; i++) {
    ...
    line.replace(regex.entity, function(xml, value){
      split_translatable_value(value, translatable_value_separator).forEach(chunk => {
        translatable.entity.push({index: i, value: chunk})
      })
      return ''
    })
