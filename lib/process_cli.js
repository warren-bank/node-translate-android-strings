...
const reset_global_regex = function(global_regex, initial_offset=0) {
  global_regex.lastIndex = initial_offset
  return global_regex
}

const split_translatable_value = function(value, separator = regex.split.all) {
  reset_global_regex(separator)
  reset_global_regex(regex.trim.whitespace_or_punctuation_from_start)
  reset_global_regex(regex.trim.whitespace_or_punctuation_from_end)
...
const remove_escape_characters = function(str) {
  reset_global_regex(regex.format.has_escape)
...
const add_escape_characters = function(str) {
  reset_global_regex(regex.format.non_escape)
...
const process_input_line = function(i, initial_offset, line, argv_vals, translatable_value_separator, array_context) {
  reset_global_regex(translatable_value_separator)

  if (array_context > 0) {
    const matches = reset_global_regex(regex.string_array.tag_end, initial_offset).exec(line)
    ...
    if (array_context === 1) {
      ...
      reset_global_regex(regex.string_array.item, initial_offset)
...

  {
    const matches = []
    let match
    reset_global_regex(regex.entity, initial_offset)
  ...
  reset_global_regex(regex.string, initial_offset)
  ...
  reset_global_regex(regex.string_array.one_liner, initial_offset)
  line.replace(regex.string_array.one_liner, function(xml, xml_attributes, items, array_offset) {
    ...
    reset_global_regex(regex.string_array.item, items_offset)
    items.replace(regex.string_array.item, function(xml, value, item_offset) {
  ...
  //if (array_context === 0)
  {
    const matches = reset_global_regex(regex.string_array.tag_start, initial_offset).exec(line)
...
const get_output_text_translation = function(argv_vals, translations) {
  ...
  for (let i=0; i < all_lines.length; i++) {
    ...
    if (argv_vals["--no-comments"] && reset_global_regex(regex.filter.xml_comment).test(line)) {
      line     = line.replace(reset_global_regex(regex.filter.xml_comment), '')
      filtered = true
    }
